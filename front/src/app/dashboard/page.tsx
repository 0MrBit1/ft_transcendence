...
export default function UserDashboard() {
  ...
  const fetchData = async () => {
    ...
    if (rsvpsRes.data) {
      ...
      rsvpsRes.data.forEach((r: { event_id: string; status: string }) => { rMap[r.event_id] = r.status; });
      setMyRsvps(rMap);
    }

    if (allRsvpsRes.data) {
      const counts: Record<string, number> = {};
      allRsvpsRes.data.forEach((r: { event_id: string }) => { counts[r.event_id] = (counts[r.event_id] || 0) + 1; });
                ...
                {upcomingEvents.map((event, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="bg-card rounded-xl border border-border p-5 flex flex-col shadow-sm"
                    >
              ...
              <div className="space-y-3">
                {myOrgs.map((org, i) => (
                  <motion.div
                    ...
                    className="bg-card rounded-xl border border-border p-5 flex items-center justify-between cursor-pointer hover:bg-muted/30 transition-colors shadow-sm"
                  >
